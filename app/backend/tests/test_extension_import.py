...
from extension_import import (
    ...
    JobPageError,
    _failed,
    classify_job_description_unavailable,
...
def test_html_is_sanitized_to_readable_job_text() -> None:
    text = extract_job_description(
        "<nav>Home Cookie settings</nav><h1>Senior Engineer</h1>"
        "<p>Build reliable systems.</p><ul><li>Python</li>"
        "<li>Distributed systems</li></ul><script>alert(1)</script>"
...
@pytest.mark.parametrize(
    "url", ["javascript:alert(1)", "file:///etc/passwd", "not a url"]
)
    ...
    request = ImportRequest.model_validate(
        {
            ...
            "linkedin_post_text": "Hiring engineers",
            "job_description_url": "https://jobs.example.com/1",
...
def test_success_persists_all_capture_fields_and_queue_relationship(
    ...
    class Storage(ExtensionImportTestStorage):
        ...
        def append_extension_processing_queue_item(
            ...
            return {
                ...
                "queue_item_id": "00000000-0000-0000-0000-000000000002",
                "outreach_item_id": "00000000-0000-0000-0000-000000000003",
                "queue_item_count": 1,
    ...
    result = import_capture(
        ImportRequest.model_validate(
            {
                "version": 1,
                "linkedin_post_url": "https://www.linkedin.com/feed/1",
                "author_name": "Ada",
                "author_profile_url": "https://www.linkedin.com/in/ada",
                "linkedin_post_text": "A post",
                "job_description_url": "https://jobs.example.com/1",
                "job_description_text": (
                    "Visible job description captured in the browser."
                ),
                "job_description_source": "visible_page",
                "captured_at": "2026-07-19T00:00:00Z",
            }
        ),
        authenticated_user(),
    ...
    assert (
        ...
        and saved["author_name"]
        and saved["author_profile_url"]
    )
    assert (
        saved["linkedin_post_text"]
        and saved["job_description_url"]
        and saved["job_description_text"]
    )
        ...
        (
            {
                ...
                "queue_id": "00000000-0000-0000-0000-000000000011",
                "outreach_item_id": "00000000-0000-0000-0000-000000000012",
                "record_type": "processing_queue_item",
                "location": "Already in Processing Queue",
                "open_path": "/processing-queue?queueId=x",
            },
            "Already in Processing Queue",
            "/processing-queue?queueId=x",
        ),
        (
            {
                "id": "00000000-0000-0000-0000-000000000010",
                "queue_id": "00000000-0000-0000-0000-000000000011",
                "outreach_item_id": "00000000-0000-0000-0000-000000000012",
                "record_type": "failed_task",
                "location": "Failed Tasks",
                "open_path": "/failed-tasks",
            },
            "Failed Tasks",
            "/failed-tasks",
        ),
        (
            {
                ...
                "location": "Already in Review Queue",
...
def test_existing_visible_records_return_location_and_open_path(
    ...
    assert (
        ...
        and result.existing_item_path == path
    )
...
def test_orphaned_outreach_is_repaired_instead_of_reported_as_duplicate(
    ...
    class Storage(ExtensionImportTestStorage):
        ...
        def repair_extension_orphan(self, *_: object) -> dict[str, object]:
            return {
                "queue_id": "00000000-0000-0000-0000-000000000001",
                "queue_item_id": "00000000-0000-0000-0000-000000000002",
                "outreach_item_id": "00000000-0000-0000-0000-000000000012",
                "queue_item_count": 1,
                "created_new_queue": False,
                "queue_status": "draft",
            }
...
def test_empty_orphan_repair_response_falls_back_to_a_new_import(
    ...
    class Storage(ExtensionImportTestStorage):
        ...
        def repair_extension_orphan(self, *_: object) -> None:
            return None
    ...
    request = ImportRequest.model_validate(
        {
            ...
            "author_name": "Ada",
            "linkedin_post_text": "A post",
            "idempotency_key": "capture-empty-repair",
            "captured_at": "2026-07-19T00:00:00Z",
        }
    )
...
def test_duplicate_url_is_normalized_before_lookup(
    ...
    monkeypatch.setattr(
        "extension_import.normalize_public_url",
        lambda _url: "https://www.linkedin.com/feed/1",
    )
    request = ImportRequest.model_validate(
        {
            "version": 1,
            "linkedin_post_url": "https://www.linkedin.com/feed/1?tracking=x#fragment",
            "author_name": "Ada",
            "linkedin_post_text": "A post",
            "job_description_url": "https://jobs.example.com/1",
            "captured_at": "2026-07-19T00:00:00Z",
...
def test_missing_jd_is_queued_and_does_not_create_failed_task(
    ...
    class Storage(ExtensionImportTestStorage):
        ...
        def append_extension_processing_queue_item(
            self, _user: str, metadata: dict[str, object]
        ) -> dict[str, object]:
            assert (
                metadata["job_description_text"] is None
                and metadata["job_description_source"] == "unavailable"
            )
            return {
                "queue_id": "00000000-0000-0000-0000-000000000001",
                "queue_item_id": "00000000-0000-0000-0000-000000000002",
    ...
    assert (
        ...
        and result.outreach_item_id
    )


def test_exact_blocked_jd_workflow_queues_without_failed_or_existing_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    saved: dict[str, object] = {}
    ...
    monkeypatch.setattr(
        "extension_import.normalize_public_url",
        lambda url: url.split("?")[0].rstrip("/"),
    )
    request = ImportRequest.model_validate(
        {
            "version": 1,
            "linkedin_post_url": "https://www.linkedin.com/feed/update/urn:li:activity:123?trk=feed",
            "author_name": "Ada",
            "author_profile_url": "https://www.linkedin.com/in/ada",
            "linkedin_post_text": "Hiring platform engineers",
            "job_description_url": "https://jobs.example.com/blocked",
            "job_description_text": None,
            "job_description_source": "unavailable",
            "idempotency_key": "capture-12345678",
            "captured_at": "2026-07-19T00:00:00Z",
        }
    )
    result = import_capture(request, authenticated_user(), Storage())
    ...
    assert (
        saved["job_description_url"] == "https://jobs.example.com/blocked"
        and saved["job_description_text"] is None
    )
    ...
    request = ImportRequest.model_validate(
        {
            ...
            "linkedin_post_text": "A post",
            "job_description_source": "unavailable",
            "idempotency_key": "capture-failed-123",
            "captured_at": "2026-07-19T00:00:00Z",
        }
    )
    assert import_capture(request, authenticated_user(), Storage()).outcome == "queued"
...
def test_queue_append_error_is_reported_without_creating_a_failed_task(
    ...
    class Storage(ExtensionImportTestStorage):
        ...
        def append_extension_processing_queue_item(
            ...
            raise ExtensionQueueAppendError(
    ...
    request = ImportRequest.model_validate(
        {
            ...
            "author_name": "Ada",
            "linkedin_post_text": "A post",
            "idempotency_key": "capture-queue-full",
            "captured_at": "2026-07-19T00:00:00Z",
        }
    )
    result = import_capture(request, authenticated_user(), Storage())
...
@pytest.mark.parametrize(
    ("status_code", "body", "content_type", "timed_out"),
    [
        (401, "", "text/html", False),
        (403, "", "text/html", False),
        (429, "", "text/html", False),
        (200, "CAPTCHA Verify you are human", "text/html", False),
        (200, "Access denied", "text/html", False),
        (200, "Login required", "text/html", False),
        (200, "", "application/pdf", False),
        (None, "", None, True),
    ],
)
def test_protected_or_unavailable_jd_outcomes_are_nonfatal(
    status_code: int | None, body: str, content_type: str | None, timed_out: bool
) -> None:
    assert classify_job_description_unavailable(
        status_code, body, content_type, timed_out
    )
    ...
    request = ImportRequest.model_validate(
        {
            ...
            "author_name": "Ada",
            "linkedin_post_text": "A post",
            "job_description_text": "Manually pasted description",
...
def test_terminal_failure_is_verified_before_failed_response_is_returned() -> None:
    class Storage(ExtensionImportTestStorage):
        def create_extension_failed_task(
            ...
            assert (
                user_id == "user-1"
                and metadata["linkedin_post_url"] == "https://www.linkedin.com/feed/1"
            )
        ...
        def get_failed_task(self, item_id: str, user_id: str) -> dict[str, object]:
            ...
            return {
                ...
                "failure_stage": "job_description_fetch",
            }
    ...
    result = _failed(
        ...
        {
            "linkedin_post_url": "https://www.linkedin.com/feed/1",
            "job_description_url": "https://jobs.example/1",
        },
        "unreachable",
        "job_description_fetch",
    )
...
def test_unverified_failed_insert_never_claims_failed_tasks() -> None:
    class Storage(ExtensionImportTestStorage):
        ...
        def get_failed_task(self, *_: object) -> None:
            return None
