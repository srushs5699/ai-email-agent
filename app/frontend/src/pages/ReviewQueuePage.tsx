...
import {
  approveDraft, deleteDraft, listReviewDrafts, regenerateDraft, rejectDraft,
...
type BusyAction = 'saving' | 'regenerating' | 'sending' | null
...
export function ReviewQueuePage() {
  ...
  async function remove(draft: Draft, action: 'reject' | 'delete'): Promise<void> {
    ...
    setBusy((current) => ({ ...current, [draft.id]: 'saving' }))
    try { if (action === 'reject') await rejectDraft(draft.id); else await deleteDraft(draft.id); setDrafts((current) => current.filter((item) => item.id !== draft.id)) } catch { setError(`Unable to ${action} this draft.`) } finally { setBusy((current) => ({ ...current, [draft.id]: null })) }
  }
