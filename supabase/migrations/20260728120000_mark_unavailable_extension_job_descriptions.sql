-- Step 10: JD retrieval is browser-visible/manual only. Missing text is valid.
...
begin
  ...
  insert into public.outreach_items(
    ...
    nullif(p_metadata->>'job_description_url',''), nullif(p_metadata->>'job_description_text',''),
    coalesce(nullif(p_metadata->>'job_description_text',''), '') = '', 'captured'
  ) returning * into o;
  ...
  insert into public.processing_queue_items(
    ...
    coalesce(p_metadata->>'job_description_source','unavailable'),(p_metadata->>'captured_at')::timestamptz
  ) returning * into i;
