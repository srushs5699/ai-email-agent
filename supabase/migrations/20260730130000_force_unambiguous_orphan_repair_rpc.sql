...
drop function if exists public.repair_extension_orphaned_outreach(text, text, jsonb);

create function public.repair_extension_orphaned_outreach(
  p_user_id uuid,
  p_outreach_item_id uuid,
  p_metadata jsonb
) returns table(
  queue_id uuid,
  queue_item_id uuid,
  outreach_item_id uuid,
  queue_item_count integer,
  created_new_queue boolean,
  queue_status text
)
...
begin
  ...
  where oi.id = p_outreach_item_id and oi.user_id = p_user_id
  returning oi.* into o;
  ...
  select * into q from public.processing_queues as pq
  ...
  values (q.id,p_user_id,q.total_items,p_metadata,o.id,o.linkedin_post_url,o.linkedin_author_name,o.linkedin_author_profile_url,o.linkedin_post_text,o.job_description_url,o.job_description_text,coalesce(p_metadata->>'capture_source','browser_extension'),(p_metadata->>'captured_at')::timestamptz)
  returning * into i;
  update public.processing_queues as pq set total_items = pq.total_items + 1
    where pq.id = q.id returning pq.status,pq.total_items into queue_status,queue_item_count;
